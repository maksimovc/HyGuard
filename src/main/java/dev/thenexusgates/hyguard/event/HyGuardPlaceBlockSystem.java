...
public final class HyGuardPlaceBlockSystem extends EntityEventSystem<EntityStore, PlaceBlockEvent> {

    private final HyGuardPlugin plugin;

    public HyGuardPlaceBlockSystem(HyGuardPlugin plugin) {
    ...
    public void handle(int index,
        ...
        if (!plugin.evaluate(playerRef, world.getName(), BlockPos.fromVector(event.getTargetBlock()), ProtectionAction.BLOCK_PLACE).allowed()) {
            event.setCancelled(true);
